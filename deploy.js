		...
		req.on('end', () => {
			...
			if (url.pathname === '/deploy') {
				...
				const commands = [
					...
					'cp -r frontend/dist/* /var/www/market/ 2>/dev/null || echo "⚠️ Нет файлов для копирования (возможно, сборка не выполнена)"',
					...
					// Перезапуск PM2
					'cd ..',
					'pm2 restart market || echo "⚠️ Ошибка перезапуска market"',
					`pm2 restart market-deploy:${PORT} --update-env || echo "⚠️ Ошибка перезапуска деплой-сервера"`,
				];
